...
`.codex/scripts/validate-orchestration.ps1` valida a configuracao, os dez agentes, a politica A-D, o guard de seguranca e a simulacao de roteamento. Use `-RunGradle` para incluir a bateria Gradle e `-CheckAdb` para consultar dispositivo autorizado.
...
| C - complexa | multiplos modulos, contratos, lifecycle, OCR estrutural | Sol max/high para decisao; Terra para execucao delimitada | `architect`, `debugger`, implementador, `tester` |
| D - critica | privacidade, seguranca, MediaProjection, release, dados pessoais | Sol alto/max | `architect`, `privacy_security`, `release_gate`, `reviewer` |
...
| Investigar falha intermitente de MediaProjection | C/D | `debugger`, `architect`, `privacy_security` | Terra alto + Sol alto/max | Lifecycle e permissao sensivel. |
| Preparar release | D | `release_gate`, `privacy_security`, opcional `reviewer` | Sol alto + Terra alto | Gate independente do implementador. |
