...
private val Metric.targetLabel: String
    get() = when (this) {
        ...
        Metric.RATE_PER_KM, Metric.RATE_PER_HOUR -> "Valor desejado"
...
private val Metric.inputUnit: String
    get() = when (this) {
        Metric.PAYOUT, Metric.RATE_PER_KM, Metric.RATE_PER_HOUR -> "R$"
...
private fun Long.toDisplayInput(metric: Metric): String = when (metric) {
    Metric.PAYOUT, Metric.RATE_PER_KM, Metric.RATE_PER_HOUR -> (toDouble() / 100).formatForInput(2)
...
private fun String.toNormalizedTarget(metric: Metric): Long? {
    ...
    val multiplier = when (metric) {
        Metric.PAYOUT, Metric.RATE_PER_KM, Metric.RATE_PER_HOUR, Metric.PASSENGER_RATING -> 100
