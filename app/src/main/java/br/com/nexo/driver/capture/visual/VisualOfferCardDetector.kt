 ...
 *
 * It samples horizontal bands through the lower 35–76% of the frame. A real offer card creates
...
class OfferCardVisualDetector(
    ...
    /** [providerHint] is intentionally non-authoritative: card geometry is provider-agnostic. */
    fun detect(frame: VisualPixelFrame, @Suppress("UNUSED_PARAMETER") providerHint: String? = null): VisualCardRegion {
        val fallback = fallback(frame.width, frame.height)
    ...
    fun fallback(width: Int, height: Int): VisualCardRegion = fallbackCrop(width, height)
    ...
    private fun fallbackCrop(width: Int, height: Int): VisualCardRegion {
        val marginX = (width * HORIZONTAL_MARGIN_FRACTION).toInt().coerceAtLeast(1)
        val top = (height * FALLBACK_TOP_FRACTION).toInt().coerceAtMost(height - 1)
    ...
    private companion object {
        ...
        const val SEARCH_TOP_FRACTION = 0.35f
