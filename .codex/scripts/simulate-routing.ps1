...
$routes = @(
    ...
    [pscustomobject]@{
        Class = "C"
        Scenario = "Mudanca transversal, lifecycle, contrato ou bug sem causa clara"
        Agents = "architect or debugger, implementer, tester"
    ...
    [pscustomobject]@{
        Class = "D"
        Scenario = "Privacidade, seguranca, MediaProjection, release ou risco irreversivel"
        Agents = "architect/privacy_security/release_gate + optional reviewer"
        Models = "gpt-5.6-sol:high/max + gpt-5.6-terra:high"
        MaxAgents = 4
        UsesSol = $true
    }
)
...
if ($Validate) {
    ...
    if (($routes | Where-Object { $_.Class -eq "D" }).Agents -notmatch "privacy_security") {
        $errors += "Class D must include privacy_security."
    }
